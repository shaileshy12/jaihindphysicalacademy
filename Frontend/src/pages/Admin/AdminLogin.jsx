...
const AdminLogin = () => {
  const [isResetMode, setIsResetMode] = useState(false); // This is the toggle!
  const [formData, setFormData] = useState({ email: "", password: "", recoveryKey: "" });
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-800 flex items-center justify-center p-4">
      <div className="bg-white/10 backdrop-blur-xl rounded-2xl p-8 max-w-md w-full border border-white/20">
        ...
        <form onSubmit={isResetMode ? handleReset : handleLogin} className="space-y-4">
          ...
          {/* Only show Master Key input if they are resetting */}
          {isResetMode && (
            <div>
              <label className="block text-sm font-medium text-red-300 mb-2">Master Recovery Key</label>
              <input
                type="password"
                required
                className="w-full p-3 rounded-xl bg-red-900/20 border border-red-400/50 text-white focus:outline-none focus:ring-2 focus:ring-red-400"
          ...
          >
            {loading ? "Processing..." : (isResetMode ? "Reset & Save" : "Login")}
          </button>
        </form>
